...
function proses($mysqli){
    ...
    $sales_dataX = $mysqli->query("select id_store,date,due_date,status,id_customer,no_invoice from sales_data where $mysqli->user_master_query and no_invoice = '".$mysqli->real_escape_string($no_invoice)."'");
    $sales_data = null;
    while ($row = $sales_dataX->fetch_assoc()) {
        $sales_data = $row;
    }
    $salesX = $mysqli->query("select no_invoice from sales where $mysqli->user_master_query and id_product = '".$mysqli->real_escape_string($id_product)."' and no_invoice = '".$mysqli->real_escape_string($no_invoice)."'");
    $sales = null;
