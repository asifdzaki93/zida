...
function proses($mysqli){
    ...
    $sales_dataX = $mysqli->query("select totalpay from sales_data where $mysqli->user_master_query and no_invoice = '".$mysqli->real_escape_string($no_invoice)."'");
    $sales_data = null;
    ...
    $salesX = $mysqli->query("select totalprice,price,id_sales from sales where $mysqli->user_master_query and no_invoice = '".$mysqli->real_escape_string($no_invoice)."'");
    $totalorder=0;
    while ($row = $salesX->fetch_assoc()) {
        ...
        $mysqli->query("UPDATE `sales` SET `amount`='$amount',`totalprice`='$totalprice' where `id_sales` = '".$sales["id_sales"]."'");        
    }
    ...
    $updateSQL = "UPDATE `sales_data` SET `totalorder`='$totalorder', `note` = '$note', `due_date` = '$due_date' where `no_invoice` = '".$no_invoice."'";
