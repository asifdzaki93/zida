...
function formatRupiah(number) {
    ...
    if (sisa > 0) {
      ...
      return 'Rp. ' + rupiah + ',' + (desimal < 10 ? '0' : '') + desimal;
    }
     ...
     if (dt_invoice_table.length) {
       var dt_invoice = dt_invoice_table.DataTable({
          serverSide: true, // Aktifkan server-side processing
             ajax: {
          url: 'api/invoice-list.php',
          type: 'GET', // Atau 'POST', sesuaikan dengan yang server Anda terima
                  error: function (xhr, error, code) {
              console.log(error);  // Add error logging to help diagnose issues
      },
         columns: [
         ...
         columnDefs: [
           ...
           },
           {
             ...
             render: function (data, type, full, meta) {
               var $invoice_status = full['invoice_status'],
                 ...
                 $balance = formatRupiah(full['balance']);
               var roleBadgeObj = {
                ...
                'paid off': '<span class="avatar avatar-sm"> <span class="avatar-initial rounded-circle bg-label-warning"><i class="mdi mdi-chart-pie-outline"></i></span></span>',
                 'pre order':
           ...
           },
           {
             ...
             render: function (data, type, full, meta) {
               ...
               if ($image === true) {
                 // For Avatar image
                 var $output =
                   '<img src="' + 'https://zieda.id/pro/geten/images/' + $user_img + '" alt="Avatar" class="rounded-circle">';
               } else {
                 // For Avatar badge
           ...
           },
           {
             ...
             targets: 5,
             render: function (data, type, full, meta) {
               var $total = formatRupiah(full['total']);
           ...
           },
           {
             ...
             render: function (data, type, full, meta) {
               ...
               var $totale = full['total'];
               var formattedBalance = ($totale-$balance); // Memformat balance ke rupiah
               if ($balance <= $totale) {
                 var $badge_class = 'bg-label-success';
                 return '<span class="badge rounded-pill ' + $badge_class + '" text-capitalized> Lunas </span>';
               } else {
                 return '<span class="d-none">' + formattedBalance + '</span>' + formatRupiah(formattedBalance);
               }
             }
           },
           {
